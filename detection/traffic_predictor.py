...
# Fixed 60s horizon (matches training HORIZON x 3s)
_PREDICTION_HORIZON_SEC = 60
...
class TrafficDensityPredictor:
    ...
    def __init__(self, model_paths: dict[str, Path] | None = None) -> None:
        ...
        if not model_paths:
            raise ValueError(
                "No model paths provided. Cannot initialize TrafficDensityPredictor."
            )
        ...
        print(
            f"✓ Successfully loaded all {len(self._models)} ML models for density prediction"
        )
